...
var
  TextoAtual: string;
begin
  // Obter o texto atual do campo editCPF
  TextoAtual := editCPF.Text;

  // Verificar se o caractere digitado é um número ou um dos caracteres de formatação
  ...
  // Verificar o comprimento do texto inserido no campo editCPF
  if Length(TextoAtual) <= 11 then
  begin
    // Aplicar a máscara de CPF (###.###.###-##)
    if (Length(TextoAtual) = 3) or (Length(TextoAtual) = 7) then
  ...
  begin
    ...
    if (Length(TextoAtual) = 2) or (Length(TextoAtual) = 6) then
      TextoAtual := TextoAtual + '.';
    if (Length(TextoAtual) = 10) or (Length(TextoAtual) = 15) then
      TextoAtual := TextoAtual + '/';
    if Length(TextoAtual) = 18 then
      TextoAtual := TextoAtual + '-';
  end;

  // Atualizar o texto no campo editCPF
  editCPF.Text := TextoAtual;
end;
